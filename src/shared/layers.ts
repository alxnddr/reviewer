...
/** `buildTree`, memoized on the `layers` array's identity. Every call site here —
 * `layerOutline` and `layerSubtree` — re-derives the same tree from the same array, and
 * callers upstream (`overview.ts`'s per-comment `layerOwning`, `review-export.ts`'s per-comment
 * `layerIndexOfComment`, `read-progress.ts`'s per-layer tallies) call one of those once per
 * item in a loop. `layers` is memoized upstream (`DiffScreen.tsx`, `LayerList.tsx`),
 * so its identity is stable for as long as its content is — the same contract `fileSignatures`
 * in `read-progress.ts` relies on — which makes identity an honest cache key here too. */
const treeCache = new WeakMap<readonly ReviewLayer[], LayerTree>();

function getTree(layers: readonly ReviewLayer[]): LayerTree {
  const cached = treeCache.get(layers);
  if (cached !== undefined) {
    return cached;
  }
  const tree = buildTree(layers);
  treeCache.set(layers, tree);
  return tree;
}

/** The outline, in the artifact's own order. */
export function layerOutline(layers: readonly ReviewLayer[]): LayerOutlineEntry[] {
  const tree = getTree(layers);
...
function layerSubtree(layer: ReviewLayer, layers: readonly ReviewLayer[]): ReviewLayer[] {
  const tree = getTree(layers);
