 ...
 *
 * This asks the hunk list a different question than `coversRange` does — "which hunk holds
 * this line" rather than "does one hunk cover this range" — so the unified walk (task 034)
 * kept both: they share `hunkSpan` (walk.ts), which is the whole of the geometry either of
 * them needs. */
